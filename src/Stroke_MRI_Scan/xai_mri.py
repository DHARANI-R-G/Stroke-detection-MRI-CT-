...
DATASET_DIR = r"D:\Strokes_MRI_Balanced_data"
NUM_CLASSES = 3
BATCH_SIZE = 32
EPOCHS = 17
LR = 3e-5
XAI_DIR = r"D:\XAI_vit_mri_3"
...
def generate_region_description(sorted_regions, top_n=3):
    ...
    return text + " and determined these structures appear normal, contributing to the overall classification."
...
def generate_xai_visualization(model, img_tensor, img_np, prediction, confidence, true_label, img_index):
    class_names = ['Normal', 'Ischemic', 'Haemorrhagic']
    ...
    ax1.imshow(img_np)
    ax1.set_title('Original MRI Scan', fontweight='bold')
    ax1.axis('off')
    ...
    if prediction == 0:
        ...
        modified_desc = f"The AI examined the {top_region} closely and detected potential abnormalities, contributing to the '{class_names[prediction]}' classification."
    ...
    if prediction != 0:
        ...
        text_content.append(f"  ALERT: STROKE DETECTED ({class_names[prediction]}) with {certainty}")
        if attention_map is not None:
...
results_file = os.path.join("results", "metrics_stroke_mri.txt")
